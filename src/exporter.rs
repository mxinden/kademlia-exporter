...
impl Exporter {
    ...
    fn record_event(&self, event: client::Event) {
        match event {
            client::Event::Mdns(event) => {
                match event {
                    MdnsEvent::Discovered(_) => {
                        self.metrics.event_counter
...
impl Metrics {
    fn register(registry: &Registry) -> Metrics {
        let event_counter = CounterVec::new(Opts::new(
            "network_behaviour_event",
            "Libp2p network behaviour events.",
        )
                                            .variable_labels(vec!["behaviour".to_string(), "event".to_string()]), &["behaviour", "event"]).unwrap();
        registry.register(Box::new(event_counter.clone())).unwrap();

        let bucket_size = Gauge::with_opts(Opts::new("kad_kbuckets_size", "Libp2p Kademlia K-Buckets size.")).unwrap();
        registry
            .register(Box::new(bucket_size.clone()))
            .unwrap();
        Metrics {
            event_counter,
            bucket_size,
